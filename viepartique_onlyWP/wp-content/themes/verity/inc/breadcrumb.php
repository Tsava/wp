 ...
 * @since Verity 0.1
 */
if ( !function_exists( 'verity_breadcrumb' ) ) :
	function verity_breadcrumb() {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'verity' ); // text for the 'Home' link
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'verity' ); // text for a category page
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'verity' ); // text for a search results page
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'verity' ); // text for a tag page
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'verity' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'verity' ); // text for the 404 page
		...
		if ( ! is_front_page() ) {
			...
			elseif ( is_day() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'verity' ) ) ) ), get_the_time( __( 'Y', 'verity' ) ) ) ;
				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'verity' ) ), get_the_time( __( 'm', 'verity' ) ) ) ), get_the_time( __( 'F', 'verity' ) ) );
				echo $before . get_the_time( __( 'd', 'verity' ) ) . $after;

			}
			elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'verity' ) ) ) ), get_the_time( __( 'Y', 'verity' ) ) ) ;
				echo $before . get_the_time( __( 'F', 'verity' ) ) . $after;

			}
			elseif ( is_year() ) {
				echo $before . get_the_time( __( 'Y', 'verity' ) ) . $after;
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'verity' ), max( $paged, $page ) );
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) {
					echo ')';
				}
			}
